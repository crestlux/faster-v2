#!/usr/bin/env bash
# tool_hang (ToolHang, horizon=700) — ACTION CHUNK: chunk_size=8, exec_horizon=4. image + proprioception.
#   online step(max_steps/eval_interval)을 baseline과 동일 값으로 둠(=결정/update/wandb step 축 일치).
#   주의: 동일 max_steps에서 ac는 4× env step 실행(더 많은 실데이터·더 긴 wallclock). 가장 어려운 task → 예산은 시작점.
